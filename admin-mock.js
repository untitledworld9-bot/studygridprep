...
window.addQuestionBlock = () => {
  ...
  // Add to draft array
  STATE.draftQuestions.push({ _blockId: idx });
  refreshActiveTestBanner();
};

window.removeQuestionBlock = idx => {
...
window.clearAllQuestions = async () => {
  const yes = await confirmModal('Clear All Questions', 'Remove all question blocks from the builder? (Not yet uploaded to Firebase)');
  if (!yes) return;
  const wrap = $('qBuilderWrap');
...
/** Collect all question blocks from DOM into STATE.draftQuestions */
...
window.publishTest = async () => {
  ...
  try {
    ...
    // Update test status in separate write
...
function listenActiveUsers() {
  ...
  STATE.unsubActive = onSnapshot(
    ...
    snap => {
      STATE.liveUsers = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      renderLiveUsers();
      updateDashboardStats();
    },
...
window.doLogout = async () => {
  ...
  if (!yes) return;
  try {
    [STATE.unsubTests, STATE.unsubActive, STATE.unsubHistory,
