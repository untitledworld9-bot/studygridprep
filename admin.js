...
function updateUserStats() {
  ...
  // isLiveUser — status online or focusing AND lastActive within 5 min
    ...
    <tr>
      ...
      <td>
        <div style="font-family:var(--font-mono);font-size:12px;">
          <div style="color:var(--accent-amber);">⚡ ${xp} XP</div>
...
function renderOnlineUsersModal() {
  ...
  listEl.innerHTML = list.map(u => {
    ...
    const pageLabel = (() => {
      ...
      if (p.includes("home") || p === "index" || p.includes("index")) return "🏠 Home";
      return `📄 ${pageName}`;
    })();
...
window.sendAnnouncement = async () => {
  ...
  const userVal  = ($("announceUser")?.value     || "").trim();
  const btn      = $("announceBtn");

  if (!text) { toast("Please write an announcement first.", "warning"); return; }
  if (target === "user" && !userVal) { toast("Please enter a target username or email.", "warning"); return; }
  ...
  try {
    await addDoc(collection(db, COLL.ANNOUNCEMENTS), {
      ...
      priority,
      target:    target === "user" ? userVal : target,
      user:      target === "user" ? userVal : null,
...
window.sendPromotion = async () => {
  ...
  const targetType = $("promoTarget")?.value       || "all";
  const userVal  = ($("promoUser")?.value          || "").trim();
  const btn      = $("promoBtn");

  if (!title && !body) { toast("Please fill in title or message.", "warning"); return; }
  if (targetType === "user" && !userVal) { toast("Please enter a target username or email.", "warning"); return; }
  ...
  try {
    await addDoc(collection(db, COLL.PROMOTIONS), {
      ...
      target:    targetType === "user" ? userVal : "all",
      user:      targetType === "user" ? userVal : null,
      active:    true,
      time:      Date.now(),
      createdAt: serverTimestamp()
    });
...
/** Full Leaderboard: sorted by XP from leaderboard collection, shows Level + Badge */
function renderFullLeaderboard(container) {
  // Use leaderboard collection if available (has real XP from uw-core.js)
  // Merge status info from STATE.allUsers
  let users;
  if (STATE.leaderboardData && STATE.leaderboardData.length > 0) {
    users = [...STATE.leaderboardData]
      ...
      .slice(0, 100)
      .map(lb => {
        // Merge status from allUsers
        const u = STATE.allUsers.find(u => u.id === lb.id) || {};
        return { ...u, ...lb, id: lb.id };
      });
  } else {
    users = [...STATE.allUsers]
      .sort((a, b) => {
        const axp = Number(a.xp ?? a.totalXP ?? a.points ?? a.score ?? 0);
        const bxp = Number(b.xp ?? b.totalXP ?? b.points ?? b.score ?? 0);
        return bxp - axp;
      })
      .slice(0, 100);
  }
  ...
  container.innerHTML = `
    <table style="width:100%;border-collapse:collapse;">
      ...
      <tbody>
        ${users.map((u, i) => {
          const xp     = Number(u.xp ?? u.totalXP ?? u.points ?? u.score ?? 0);
          const level  = u.level ? Number(u.level) : adminGetLevel(xp);
          const badge  = adminGetBadge(xp);
          const streak = u.streak ?? u.currentStreak ?? u.loginStreak ?? 0;
          ...
          <tr style="border-bottom:1px solid var(--border);transition:background .15s;" onmouseover="this.style.background='rgba(255,255,255,.03)'" onmouseout="this.style.background='transparent'">
            ...
            <td style="padding:12px;font-family:var(--font-mono);color:var(--accent-amber);font-weight:700;">⚡ ${xp}</td>
            <td style="padding:12px;">
              <span style="background:rgba(124,92,252,0.15);color:var(--accent-violet);
                border:1px solid rgba(124,92,252,0.3);border-radius:20px;
                padding:3px 12px;font-size:12px;font-weight:700;">Lv.${level}</span>
            </td>
            <td style="padding:12px;font-size:13px;">${badge}</td>
...
/** Timer Leaderboard: sorted by focusTime (minutes), shows XP + Level + Badge */
...
function formatLastActive(u) {
  ...
  // Resolve lastActive timestamp (handle Firestore Timestamp objects)
  let ts = null;
  const raw = u.lastActive || u.lastSeen || null;
  if (raw) {
    ts = raw?.toMillis ? raw.toMillis() : (typeof raw === "number" ? raw : null);
  }
  ...
  // Only show "Now" if status is online/focus AND lastActive is within 5 minutes
  const STALE_MS = 5 * 60 * 1000;
  if ((s === "online" || s.includes("focus")) && ts && (Date.now() - ts) <= STALE_MS) {
    return `<span style="color:var(--accent-green)">Now</span>`;
  }
...
window.sendVideoPromo = async () => {
  ...
  const userVal   = ($("vpUser")?.value      || "").trim();
...
function renderPerformanceSection(users) {
  ...
  // Sort by total video completions (sum of 7-day array), then by focusTime
  const ranked = [...users]
    .map(u => {
      ...
      const xp      = Number(u.xp ?? u.totalXP ?? u.points ?? 0);
      return { ...u, _videos: videos, _focus: focus, _xp: xp, _graph: graph };
    })
    .filter(u => u._videos > 0 || u._focus > 0 || u._xp > 0) // only users with activity
    .sort((a, b) => b._videos - a._videos || b._focus - a._focus || b._xp - a._xp);
    ...
    " onmouseover="this.style.borderColor='rgba(0,224,255,0.3)';this.style.transform='translateY(-2px)'"
      ...
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px;flex-wrap:wrap;">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
          <span style="background:rgba(0,224,255,.1);border:1px solid rgba(0,224,255,.2);
            color:var(--accent-cyan);border-radius:99px;padding:3px 10px;font-size:11px;font-weight:700;">
            📹 ${u._videos} videos
          </span>
