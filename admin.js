...
function renderUserTable() {
  ...
  tbody.innerHTML = rows.map(u => `
    <tr>
      ...
      <td class="mono" style="color:var(--text-muted);">${u.lastActiveDate || formatTimestamp(u.lastActive) || '—'}</td>
      <td>
...
window.sendPromotion = async () => {
  ...
  const ctaUrl   = ($("promoCTAUrl")?.value     || "").trim(); // URL for CTA button
...
window.pushUpdate = async () => {
  ...
  try {
    // Use setDoc to always overwrite the same "latest" document
...
// ============================================================
//  PROMOTIONS HISTORY — real-time listener + delete
// ============================================================
...
function renderPromotionHistory(list) {
  ...
  if (!container) return;
  if (!list.length) {
    container.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🎯</div><div class="empty-state-text">No promotions sent yet</div></div>`;
    return;
  }
  container.innerHTML = list.map(p => `
<div class="announce-item">
  <span class="announce-priority p-medium" style="background:rgba(124,92,252,.12);color:#7c5cfc;border-color:rgba(124,92,252,.2);">${escHtml(p.type || "popup")}</span>
  <div style="flex:1;">
    <div class="announce-text"><b>${escHtml(p.title || "")}</b> — ${escHtml(p.body || "")}</div>
    <div class="announce-meta">${formatTimestamp(p.time)} · CTA: ${escHtml(p.cta || "—")}${p.ctaUrl ? ` → ${escHtml(p.ctaUrl)}` : ""}</div>
  </div>
...
window.deletePromotion = async id => {
  const yes = await confirmModal("Delete Promotion", "Remove this promotion from all user feeds?");
  if (!yes) return;
  try {
...
// ============================================================
//  APP UPDATES HISTORY — real-time listener + delete
// ============================================================

function listenUpdatesHistory() {
  const unsub = onSnapshot(
    query(collection(db, COLL.UPDATES), orderBy("time", "desc"), limit(20)),
    snap => renderUpdatesHistory(snap.docs.map(d => ({ id: d.id, ...d.data() }))),
    err => console.error("Updates listener error:", err)
  );
  STATE.unsubscribers.push(unsub);
}

function renderUpdatesHistory(list) {
  const container = $("updatesHistory");
  if (!container) return;
  // Filter out the "latest" sentinel document
  const history = list.filter(u => u.id !== "latest" && u.version);
  if (!history.length) {
    container.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🔄</div><div class="empty-state-text">No updates pushed yet</div></div>`;
    return;
  }
  container.innerHTML = history.map(u => `
<div class="announce-item">
  <span class="announce-priority p-${u.type === "forced" ? "high" : "low"}">${escHtml(u.type || "optional")}</span>
  <div style="flex:1;">
    <div class="announce-text"><b>${escHtml(u.version || "")}</b></div>
    <div class="announce-meta">${formatTimestamp(u.time)}${u.changelog ? " · " + escHtml(u.changelog.substring(0, 60)) + "…" : ""}</div>
  </div>
  <button class="announce-delete" onclick="deleteUpdateHistory('${u.id}')" title="Delete">✕</button>
</div>`).join("");
}

window.deleteUpdateHistory = async id => {
...
// ============================================================
//  ROOMS — detailed view with users per room
// ============================================================

function listenRoomsDetailed() {
  // Re-use STATE.allUsers (already live via listenUsers)
  // This listener renders the rooms view whenever users update
  const unsub = onSnapshot(
    collection(db, COLL.ROOMS),
    snap => {
    ...
    err => console.error("Rooms detailed error:", err)
  );
  STATE.unsubscribers.push(unsub);
  // Also re-render whenever users change (already called from listenUsers)
}
...
function renderRoomsView() {
  ...
  if (!rooms.length) {
    container.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🏠</div><div class="empty-state-text">No rooms yet</div></div>`;
    return;
  }

  container.innerHTML = rooms.map(room => {
    const usersInRoom = STATE.allUsers.filter(u => u.room === room.id);
    const focusing    = usersInRoom.filter(u => u.status && u.status.includes("Focusing"));
    const online      = usersInRoom.filter(u => u.status === "Online");

    return `
<div class="form-panel" style="margin-bottom:16px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
    <div>
      <div style="font-weight:700;font-size:15px;">🏠 ${escHtml(room.name || room.id)}</div>
      <div style="font-size:11px;color:var(--text-muted);font-family:var(--font-mono);">ID: ${escHtml(room.id)}</div>
    </div>
    <div style="display:flex;gap:8px;">
      <span class="status-badge focusing">${focusing.length} Focusing</span>
      <span class="status-badge online">${online.length} Online</span>
    </div>
  </div>
  ${usersInRoom.length ? `
  <div style="display:flex;flex-direction:column;gap:8px;">
    ...
    <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--bg-input);border-radius:var(--radius-sm);">
      <div class="user-avatar-sm">${(u.name || "?")[0].toUpperCase()}</div>
      <div style="flex:1;font-size:13px;font-weight:600;">${escHtml(u.name || "—")}</div>
      ${statusBadge(u.status)}
      <span style="font-size:11px;color:var(--text-muted);">${formatFocusTime(u.focusTime || 0)}</span>
      <button class="btn btn-outline" style="padding:4px 10px;font-size:11px;" onclick="notifyUser('${escHtml(u.id)}','${escHtml(u.name || u.id)}')">🔔</button>
    </div>`).join("")}
  </div>` : `<div style="color:var(--text-muted);font-size:13px;text-align:center;padding:12px;">No users in this room</div>`}
</div>`;
  }).join("");

  // Re-render user leaderboard too
...
function renderAdminLeaderboard() {
  const container = $("adminLeaderboardList");
  if (!container) return;

  const sorted = [...STATE.allUsers]
    .sort((a, b) => (b.weeklyXP || 0) - (a.weeklyXP || 0))
    .slice(0, 20);

  if (!sorted.length) {
    container.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🏆</div><div class="empty-state-text">No data yet</div></div>`;
    return;
  }
...
<div class="announce-item" style="gap:14px;">
  <span style="font-size:18px;min-width:28px;text-align:center;">${medals[i] || "#" + (i+1)}</span>
  <div class="user-avatar-sm">${(u.name || "?")[0].toUpperCase()}</div>
  <div style="flex:1;">
    <div style="font-weight:600;font-size:13px;">${escHtml(u.name || "—")}</div>
    <div style="font-size:11px;color:var(--text-muted);">Focus: ${formatFocusTime(u.focusTime || 0)}</div>
  </div>
  ${statusBadge(u.status)}
  <span style="font-weight:700;color:var(--accent-amber);font-size:13px;">⭐ ${u.weeklyXP || 0} XP</span>
</div>`).join("");
}
