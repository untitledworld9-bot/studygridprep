...
function confirmModal(title, body) {
  return new Promise(resolve => {
    ...
    // FIX-DELETE: Do NOT call closeModal() here — it calls _modalResolve(false)
    // which resolves the Promise as false BEFORE resolve(true) runs.
    // Result: yes is always false → delete/action never executes.
...
window.sendAnnouncement = async () => {
  ...
  const imageUrl = ($("announceImageUrl")?.value || "").trim(); // image URL field
