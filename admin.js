...
function formatLastActive(u) {
  ...
  if (ts) {
    ...
    if (diffDay === 1) return `<span style="color:var(--text-muted)">Yesterday</span>`;
    if (diffDay < 7)   return `<span style="color:var(--text-muted)">${diffDay} days ago</span>`;
  }
...
window.sendMaintenance = async () => {
  ...
  const yes = await confirmModal(
    ...
    `This will immediately show a full-screen popup on ALL user devices that cannot be closed. Continue?`
  );
  if (!yes) return;
