...
async function parsePDFWithAI() {
  ...
  const prompt = `You are an expert at extracting exam questions from PDFs. Extract ALL MCQ questions from the provided exam paper.
...
- Extract ALL questions, preserve question numbers
- Keep full question text including any mathematical notation
...
async function parsePDFAsText(base64) {
  ...
  const prompt = `Extract ALL exam MCQ questions from this content.
Return ONLY a valid JSON array format:
[{"questionNo":1,"subject":"Physics","question":"...","options":["A","B","C","D"],"correctAnswer":"A","type":"mcq"}]
...
function renderAIPreview() {
  ...
  // Render first 20 questions (performance)
  const previewQs = qs.slice(0, 30);
  preview.innerHTML = previewQs.map((q, i) => {
    const letters = ['A','B','C','D'];
    const opts = (q.options || []).map((o, oi) =>
      `<div style="font-size:12px;padding:4px 0;${letters[oi] === q.correctAnswer ? 'color:var(--accent-green);font-weight:600;' : 'color:var(--text-muted);'}">
