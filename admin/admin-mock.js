...
function restoreQuestionBlock(data) {
  ...
  block.innerHTML = `
    ...
    <div id="qMatchWrap_${idx}" style="display:none;margin-bottom:12px;">
      <div class="form-label" style="margin-bottom:8px;">Match Columns</div>
      <div style="overflow-x:auto;border-radius:8px;border:1px solid var(--border);">
        <table style="width:100%;border-collapse:collapse;min-width:320px;">
          <thead>
            <tr>
              <th style="padding:7px 10px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--text-muted);background:rgba(0,224,255,.04);border-bottom:1px solid var(--border);width:36px;">#</th>
              <th style="padding:7px 10px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--text-muted);background:rgba(0,224,255,.04);border-bottom:1px solid var(--border);">Column I</th>
              <th style="padding:7px 10px;font-size:11px;font-weight:700;text-transform:uppercase;color:var(--text-muted);background:rgba(0,224,255,.04);border-bottom:1px solid var(--border);">Column II</th>
            </tr>
          </thead>
          <tbody id="qMatchBody_${idx}">
            ${(()=>{const abcd=['A','B','C','D'];const roman=['I','II','III','IV'];return [0,1,2,3].map(i=>`<tr>
              <td style="padding:4px 6px;border-bottom:1px solid var(--border);text-align:center;"></td>
              <td style="padding:3px;border-bottom:1px solid var(--border);"><div style="display:flex;align-items:center;gap:6px;padding:0 4px;"><span style="flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:rgba(0,224,255,.12);color:var(--accent-cyan);border:1px solid rgba(0,224,255,.25);font-size:10px;font-weight:700;">${abcd[i]}</span><input class="form-control q-left-${i}" type="text" placeholder="Enter item ${abcd[i]}" style="font-size:12px;height:32px;flex:1;"></div></td>
              <td style="padding:3px;border-bottom:1px solid var(--border);"><div style="display:flex;align-items:center;gap:6px;padding:0 4px;"><span style="flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:rgba(139,92,246,.12);color:#8B5CF6;border:1px solid rgba(139,92,246,.25);font-size:10px;font-weight:700;">${roman[i]}</span><input class="form-control q-right-${i}" type="text" placeholder="Enter item ${roman[i]}" style="font-size:12px;height:32px;flex:1;"></div></td>
            </tr>`).join('');})()}
          </tbody>
        </table>
      </div>
      <button type="button" onclick="addMatchRow(${idx})" style="margin-top:8px;font-size:11px;padding:5px 12px;background:rgba(0,224,255,.06);border:1px solid rgba(0,224,255,.15);color:var(--accent-cyan);border-radius:6px;cursor:pointer;font-family:var(--font-body);"><i class="fa-solid fa-plus"></i> Add Row</button>
      <div style="margin-top:12px;">
        <div class="form-label" style="margin-bottom:8px;font-size:11px;">Answer Choices — match A→I, B→II, C→III, D→IV</div>
        <div class="options-grid">
          <div class="opt-inp-wrap"><div class="opt-letter-badge">A</div><input class="opt-inp q-optA" type="text" placeholder="(A)-(I), (B)-(II), (C)-(III), (D)-(IV)"></div>
          <div class="opt-inp-wrap"><div class="opt-letter-badge">B</div><input class="opt-inp q-optB" type="text" placeholder="Option B"></div>
          <div class="opt-inp-wrap"><div class="opt-letter-badge">C</div><input class="opt-inp q-optC" type="text" placeholder="Option C"></div>
          <div class="opt-inp-wrap"><div class="opt-letter-badge">D</div><input class="opt-inp q-optD" type="text" placeholder="Option D"></div>
        </div>
    ...
    <div id="qTableWrap_${idx}" style="display:none;margin-bottom:12px;">
      <div class="form-label" style="margin-bottom:8px;">Table Data</div>
      <div style="overflow-x:auto;border-radius:8px;border:1px solid var(--border);">
        <table style="width:100%;border-collapse:collapse;min-width:260px;">
          <thead>
            <tr id="qDataHead_${idx}">
              ${[0,1,2].map(i=>`<th style="padding:0;border:1px solid var(--border);"><input class="form-control q-th-${i}" type="text" placeholder="Header ${i+1}" style="border:none;border-radius:0;font-size:11px;font-weight:700;text-align:center;height:32px;padding:0 8px;background:rgba(255,184,48,.06);"></th>`).join('')}
            </tr>
          </thead>
          <tbody id="qDataBody_${idx}">
            ${[0,1].map(()=>`<tr>${[0,1,2].map(()=>`<td style="padding:0;border:1px solid var(--border);"><input class="form-control q-td" type="text" placeholder="…" style="border:none;border-radius:0;font-size:12px;height:32px;padding:0 8px;"></td>`).join('')}</tr>`).join('')}
          </tbody>
        </table>
      </div>
      <div style="display:flex;gap:8px;margin-top:8px;flex-wrap:wrap;">
        <button type="button" onclick="addTableRow(${idx})" style="font-size:11px;padding:5px 12px;background:rgba(255,184,48,.06);border:1px solid rgba(255,184,48,.15);color:var(--accent-amber);border-radius:6px;cursor:pointer;font-family:var(--font-body);"><i class="fa-solid fa-plus"></i> Add Row</button>
        <button type="button" onclick="addTableCol(${idx})" style="font-size:11px;padding:5px 12px;background:rgba(255,184,48,.06);border:1px solid rgba(255,184,48,.15);color:var(--accent-amber);border-radius:6px;cursor:pointer;font-family:var(--font-body);"><i class="fa-solid fa-plus"></i> Add Column</button>
      </div>
      <div style="margin-top:12px;">
        <div class="form-label" style="margin-bottom:8px;font-size:11px;">Answer Choices — match A→I, B→II, C→III, D→IV</div>
        <div class="options-grid">
  ...
  if (typeEl) {
    typeEl.value = type;
    handleQTypeChange(typeEl, idx);
  }

  if (type === 'mcq') {
    block.querySelector('.q-optA').value = data.optionA || (data.options?.[0]) || '';
    block.querySelector('.q-optB').value = data.optionB || (data.options?.[1]) || '';
    block.querySelector('.q-optC').value = data.optionC || (data.options?.[2]) || '';
    block.querySelector('.q-optD').value = data.optionD || (data.options?.[3]) || '';
  } else if (type === 'match') {
    const left  = data.left  || [];
    const right = data.right || [];
    const tbody = block.querySelector(`#qMatchBody_${idx}`);
    if (tbody) {
      // Auto-add rows if data has more than the 4 default rows
      const needed   = Math.max(left.length, right.length);
      const existing = tbody.querySelectorAll('tr').length;
      for (let r = existing; r < needed; r++) addMatchRow(idx);
      // Fill all rows
      tbody.querySelectorAll('tr').forEach((tr, i) => {
        const lInp = tr.querySelector(`.q-left-${i}`);
        const rInp = tr.querySelector(`.q-right-${i}`);
        if (lInp) lInp.value = left[i]  || '';
        if (rInp) rInp.value = right[i] || '';
      });
    }
    const opts = data.options || [];
    ['A','B','C','D'].forEach((l,i) => {
      const el = block.querySelector(`.q-opt${l}`);
      if (el) el.value = opts[i] || '';
    });
  } else if (type === 'table') {
    const tableArr = data.table || [];
    // Figure out how many cols & rows the data needs
    const headerItem = tableArr.find(item => item.header);
    const dataRows   = tableArr.filter(item => item.row);
    const neededCols = headerItem ? headerItem.header.length : 3;
    const neededRows = dataRows.length;

    // Auto-add extra columns (default is 3)
    const existingCols = block.querySelectorAll(`#qDataHead_${idx} th`).length;
    for (let c = existingCols; c < neededCols; c++) addTableCol(idx);

    // Auto-add extra data rows (default is 2)
    const existingRows = block.querySelectorAll(`#qDataBody_${idx} tr`).length;
    for (let r = existingRows; r < neededRows; r++) addTableRow(idx);

    // Now fill the data
    let rowIdx = 0;
    tableArr.forEach(item => {
      if (item.header) {
        const ths = block.querySelectorAll(`#qDataHead_${idx} input`);
        item.header.forEach((h,i) => { if (ths[i]) ths[i].value = h; });
      } else if (item.row) {
        const trs = block.querySelectorAll(`#qDataBody_${idx} tr`);
        if (trs[rowIdx]) {
          const tds = trs[rowIdx].querySelectorAll('.q-td');
          item.row.forEach((c,i) => { if (tds[i]) tds[i].value = c; });
        }
        rowIdx++;
      }
    });
...
window.resetCreateForm = () => {
  ...
  if ($('ctIsFree')) $('ctIsFree').checked = false;
};
...
async function _saveDraftToFirestore(blockIdx) {
  ...
  if (type === 'mcq') {
    ...
    let i = 0;
    while (true) {
      const lEl = block.querySelector(`.q-left-${i}`);
      const rEl = block.querySelector(`.q-right-${i}`);
      if (!lEl && !rEl) break;
      left.push(lEl?.value.trim()  || '');
      right.push(rEl?.value.trim() || '');
      i++;
    }
    options = [
      block.querySelector('.q-optA')?.value.trim() || '',
    ...
    const headRow = block.querySelector(`#qDataHead_${blockIdx} tr`);
    if (headRow) {
      const headers = [...headRow.querySelectorAll('input')].map(inp => inp.value.trim());
      if (headers.some(h => h)) table.push({ header: headers });
    }
    const dataBody = block.querySelector(`#qDataBody_${blockIdx}`);
    if (dataBody) {
      dataBody.querySelectorAll('tr').forEach(tr => {
        const cells = [...tr.querySelectorAll('.q-td')].map(inp => inp.value.trim());
        if (cells.some(c => c)) table.push({ row: cells });
      });
    }
    options = [
      block.querySelector('.q-optA')?.value.trim() || '',
...
window.handleQTypeChange = (sel, idx) => {
  const mcqWrap   = $(`qMcqWrap_${idx}`);
  const matchWrap = $(`qMatchWrap_${idx}`);
  const tableWrap = $(`qTableWrap_${idx}`);
  if (mcqWrap)   mcqWrap.style.display   = 'none';
  if (matchWrap) matchWrap.style.display  = 'none';
  if (tableWrap) tableWrap.style.display  = 'none';
  if (sel.value === 'mcq')     { if (mcqWrap)   mcqWrap.style.display   = 'block'; }
  if (sel.value === 'match')   { if (matchWrap) matchWrap.style.display  = 'block'; }
  if (sel.value === 'table')   { if (tableWrap) tableWrap.style.display  = 'block'; }
  // integer: sab hide rehta hai
};

window.addMatchRow = function(idx) {
  const tbody = $(`qMatchBody_${idx}`);
  if (!tbody) return;
  const rowCount = tbody.querySelectorAll('tr').length;
  const tr = document.createElement('tr');
  const _abcd=['A','B','C','D','E','F','G','H'];const _roman=['I','II','III','IV','V','VI','VII','VIII'];
  const _ri=_abcd[rowCount]||String(rowCount+1);const _li=_roman[rowCount]||String(rowCount+1);
  tr.innerHTML = `
    <td style="padding:4px 6px;border:1px solid var(--border);text-align:center;"></td>
    <td style="padding:4px;border:1px solid var(--border);"><div style="display:flex;align-items:center;gap:6px;padding:0 4px;"><span style="flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:rgba(0,224,255,.12);color:var(--accent-cyan);border:1px solid rgba(0,224,255,.25);font-size:10px;font-weight:700;">${_ri}</span><input class="form-control q-left-${rowCount}" type="text" placeholder="Enter item ${_ri}" style="font-size:12px;height:32px;flex:1;"></div></td>
    <td style="padding:4px;border:1px solid var(--border);"><div style="display:flex;align-items:center;gap:6px;padding:0 4px;"><span style="flex-shrink:0;display:inline-flex;align-items:center;justify-content:center;width:22px;height:22px;border-radius:50%;background:rgba(139,92,246,.12);color:#8B5CF6;border:1px solid rgba(139,92,246,.25);font-size:10px;font-weight:700;">${_li}</span><input class="form-control q-right-${rowCount}" type="text" placeholder="Enter item ${_li}" style="font-size:12px;height:32px;flex:1;"></div></td>`;
  tbody.appendChild(tr);
};

window.addTableRow = function(idx) {
  const tbody = $(`qDataBody_${idx}`);
  const thead = $(`qDataHead_${idx}`);
  if (!tbody || !thead) return;
  const cols = thead.querySelectorAll('th').length;
  const tr = document.createElement('tr');
  tr.innerHTML = Array(cols).fill(0).map(() =>
    `<td style="padding:0;border:1px solid var(--border);"><input class="form-control q-td" type="text" placeholder="…" style="border:none;border-radius:0;font-size:12px;height:32px;padding:0 8px;"></td>`
  ).join('');
  tbody.appendChild(tr);
};
...
window.addTableCol = function(idx) {
  ...
  const colIdx = thead.querySelectorAll('th').length;
  const th = document.createElement('th');
  th.style.cssText = 'padding:0;border:1px solid var(--border);';
  th.innerHTML = `<input class="form-control q-th-${colIdx}" type="text" placeholder="Header ${colIdx+1}" style="border:none;border-radius:0;font-size:11px;font-weight:700;text-align:center;height:32px;padding:0 8px;background:rgba(255,184,48,.06);">`;
  thead.querySelector('tr').appendChild(th);
  tbody.querySelectorAll('tr').forEach(tr => {
    const td = document.createElement('td');
    td.style.cssText = 'padding:0;border:1px solid var(--border);';
    td.innerHTML = `<input class="form-control q-td" type="text" placeholder="…" style="border:none;border-radius:0;font-size:12px;height:32px;padding:0 8px;">`;
    tr.appendChild(td);
  });
};
...
function collectQuestionsFromDOM() {
  ...
  blocks.forEach((block) => {
    ...
    if (type === 'mcq') {
      ...
      const tbl = [];
      const blockId = block.id.replace('qBlock_', '');
      const headRow = block.querySelector(`#qDataHead_${blockId} tr`);
      if (headRow) {
        const headers = [...headRow.querySelectorAll('input')].map(inp => inp.value.trim());
        if (headers.some(h => h)) tbl.push({ header: headers });
      }
      const dataBody = block.querySelector(`#qDataBody_${blockId}`);
      if (dataBody) {
        dataBody.querySelectorAll('tr').forEach(tr => {
          const cells = [...tr.querySelectorAll('.q-td')].map(inp => inp.value.trim());
          if (cells.some(c => c)) tbl.push({ row: cells });
        });
      }
      q.table = tbl;
      q.left  = []; q.right = [];
      ...
      q.options = []; q.left = []; q.right = []; q.table = [];
    }
...
window.publishTest = async () => {
  ...
  try {
    ...
    // ── Step 1a: Delete OLD questions for this test (so edits replace, not duplicate) ──
    try {
      const existingSnap = await getDocs(
        query(collection(db, COLL.QUESTIONS), where('testId', '==', testId))
      );
      if (!existingSnap.empty) {
        const deleteBatches = [];
        const delChunk = 490;
        for (let i = 0; i < existingSnap.docs.length; i += delChunk) {
          const batch = writeBatch(db);
          existingSnap.docs.slice(i, i + delChunk).forEach(d => batch.delete(d.ref));
          deleteBatches.push(batch.commit());
        }
        await Promise.all(deleteBatches);
      }
    } catch(delErr) {
      console.warn('Old question cleanup (non-fatal):', delErr);
    }

    // ── Step 1b: Upload questions to mockQuestions in batches ──
...
window.applyTestFilters = () => {
  ...
  const searchVal    = ($('tmSearch')?.value || '').toLowerCase().trim();

  let list = [...STATE.allTests];
  if (examFilter)   list = list.filter(t => t.exam   === examFilter);
  if (statusFilter) list = list.filter(t => t.status === statusFilter);
  if (searchVal)    list = list.filter(t => {
    const label = buildTestLabel(t).toLowerCase();
    return label.includes(searchVal) || (t.shift||'').toLowerCase().includes(searchVal) || String(t.year||'').includes(searchVal);
  });
...
function renderTMStatsStrip() {
  const el = $('tmStatsStrip');
  if (!el) return;
  ...
  const cuet      = all.filter(t => t.exam === 'cuet' && t.status === 'published').length;
  const jee       = all.filter(t => t.exam === 'jee'  && t.status === 'published').length;
  const totalQ    = all.reduce((s, t) => s + (t.questionCount || 0), 0);

  el.innerHTML = [
    { label:'Total Tests',  val: all.length,  color:'var(--accent-cyan)'   },
    { label:'Published',    val: published,   color:'var(--accent-green)'  },
    { label:'Drafts',       val: drafts,      color:'var(--accent-amber)'  },
    { label:'Archived',     val: archived,    color:'var(--text-muted)'    },
    { label:'CUET Live',    val: cuet,        color:'var(--accent-cyan)'   },
    { label:'JEE Live',     val: jee,         color:'var(--accent-amber)'  },
    { label:'Total Qs',     val: totalQ,      color:'var(--accent-violet)' }
  ].map(s => `
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:10px;padding:12px 14px;text-align:center;">
      <div style="font-family:var(--font-display);font-size:20px;font-weight:800;color:${s.color};">${s.val}</div>
      <div style="font-size:10px;color:var(--text-muted);text-transform:uppercase;letter-spacing:.08em;margin-top:2px;">${s.label}</div>
    </div>`).join('');
}
...
function renderTestCards(tests) {
  ...
  grid.innerHTML = tests.map(t => {
    ...
    const freeBadge = t.isFree
      ? `<span class="badge badge-green" style="margin-left:6px;"><i class="fa-solid fa-unlock" style="margin-right:4px;"></i>Free</span>`
      : `<span class="badge badge-amber" style="margin-left:6px;"><i class="fa-solid fa-lock" style="margin-right:4px;"></i>Premium</span>`;
    ...
    return `<div class="test-card" onclick="viewTestDetail('${t.id}')">
      ...
      <div class="test-card-actions">
        ...
        ${t.status === 'published' || t.status === 'draft' ? `
          <button class="btn btn-secondary" style="font-size:11px;padding:6px 10px;"
            onclick="event.stopPropagation();toggleTestIsFree('${t.id}', ${!t.isFree})">
            <i class="fa-solid ${t.isFree ? 'fa-lock' : 'fa-unlock'}"></i> Mark ${t.isFree ? 'Premium' : 'Free'}
          </button>` : ''}
        ${t.status === 'published' ? `
          <button class="btn btn-secondary" style="font-size:11px;padding:6px 10px;"
  ...
  try {
    ...
    $('testDetailBody').innerHTML = `
      <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px;">
        ...
        <span class="badge ${t.isFree ? 'badge-green' : 'badge-amber'}">
          <i class="fa-solid ${t.isFree ? 'fa-unlock' : 'fa-lock'}" style="margin-right:4px;"></i>${t.isFree ? 'Free' : 'Premium'}
        </span>
...
window.toggleTestIsFree = async (id, makeFree) => {
  try {
  ...
  el.innerHTML = list.map(a => `
    <div class="announce-item">
      ...
      <div style="flex:1;">
        ...
        <div class="announce-meta" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:4px;">
          <span>${formatTimestamp(a.time)} · ${escHtml(a.target || 'all')} · 📄 ${escHtml(a.page || 'all')}</span>
  ...
  el.innerHTML = list.map(n => `
    <div class="announce-item">
      ...
      <div style="flex:1;">
        ...
        <div class="announce-meta" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:4px;">
          <span>${formatTimestamp(n.time)}${n.user ? ` · 👤 ${escHtml(n.user)}` : ''}</span>
          ${seenBadge(COLL.NOTIFICATIONS, n.id, 'Notification', n.seenBy)}
        </div>
      </div>
...
// ============================================================
//  SEEN BY MODAL — shared across all broadcast sections
// ============================================================
      ...
      return `
        <div style="display:flex;align-items:center;gap:12px;padding:10px 14px;
          ...
          <div style="width:34px;height:34px;border-radius:50%;flex-shrink:0;
            ...
            font-size:13px;font-weight:800;color:#000;">${escHtml(init)}</div>
...
// ============================================================
//  BROADCASTS — Video Promotions
// ============================================================

function listenVideoPromos() {
  if (STATE.unsubVideoPromo) STATE.unsubVideoPromo();
  STATE.unsubVideoPromo = onSnapshot(
    query(collection(db, COLL.VIDEO_PROMOS), orderBy('time', 'desc'), limit(30)),
    snap => renderVideoPromoHistory(snap.docs.map(d => ({ id: d.id, ...d.data() }))),
    err => { if (err.code !== 'permission-denied') console.warn('VideoPromos listener:', err); }
  );
}

function renderVideoPromoHistory(list) {
  const el = $('videoPromoHistory');
  if (!el) return;
  if (!list.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-state-text">No video promotions sent yet</div></div>`;
    return;
  }
  el.innerHTML = list.map(v => `
    <div class="announce-item" style="border-left:3px solid rgba(168,85,247,0.5);">
      <span style="font-size:22px;flex-shrink:0;">🎬</span>
      <div style="flex:1;min-width:0;">
        <div style="font-weight:700;font-size:13px;color:var(--accent-violet);">${escHtml(v.title || 'Video Promo')}</div>
        ${v.description ? `<div style="font-size:12px;color:var(--text-secondary);margin-top:3px;">${escHtml(v.description)}</div>` : ''}
        ${v.videoUrl ? `<div style="font-size:11px;color:var(--accent-cyan);margin-top:3px;">🎥 ${escHtml(v.videoUrl)}</div>` : ''}
        <div class="announce-meta" style="margin-top:6px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          <span>
            ...
            ${v.page && v.page !== 'all' ? ` &nbsp;·&nbsp; 📄 ${escHtml(v.page)}` : ''}
            ${v.user ? ` &nbsp;·&nbsp; 👤 ${escHtml(v.user)}` : ' &nbsp;·&nbsp; 📣 All'}
      ...
      <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end;">
        <button class="announce-delete" onclick="deleteVideoPromo('${v.id}')" title="Delete"><i class="fa-solid fa-xmark"></i></button>
        <button class="btn btn-outline" style="padding:4px 8px;font-size:10px;" onclick="toggleVideoPromoActive('${v.id}',${!v.active})">
...
window.sendVideoPromo = async () => {
  ...
  const description = ($('vpDescription')?.value || '').trim();
  const videoUrl    = ($('vpVideoUrl')?.value     || '').trim();
  const ctaUrl      = ($('vpCtaUrl')?.value       || '').trim();
  const ctaText     = ($('vpCtaText')?.value      || '').trim();
  const page        = $('vpPage')?.value          || 'all';
  const platform    = $('vpPlatform')?.value      || 'both';
  const targetType  = $('vpTarget')?.value        || 'all';
  const selUser     = ($('vpUser')?.value         || '').trim();
  const btn         = $('vpBtn');

  if (!title) { toast('Enter a video promo title.', 'warning'); return; }
  if (!videoUrl) { toast('Enter a video URL.', 'warning'); return; }
  if (targetType === 'user' && !selUser) { toast('Enter a target username.', 'warning'); return; }
  ...
  try {
    await addDoc(collection(db, COLL.VIDEO_PROMOS), {
      ...
      videoUrl, ctaUrl: ctaUrl || null, ctaText: ctaText || 'Watch Now',
      page, platform, target: finalTarget,
      user: targetType === 'user' ? selUser : null,
      active: true, time: Date.now(), createdAt: serverTimestamp()
    });
    toast('🎬 Video promo sent!', 'success');
    ['vpTitle','vpDescription','vpVideoUrl','vpCtaUrl','vpCtaText','vpUser'].forEach(id => { if ($(id)) $(id).value = ''; });
  } catch(err) { toast('Failed: ' + err.message, 'error'); }
  finally { btn.disabled = false; btn.innerHTML = '<i class="fa-solid fa-video"></i> Send Video Promo'; }
};

window.deleteVideoPromo = async id => {
  const yes = await confirmModal('Delete Video Promo', 'Remove this video promotion?');
  if (!yes) return;
  try { await deleteDoc(doc(db, COLL.VIDEO_PROMOS, id)); toast('Deleted.', 'info'); }
  catch(err) { toast('Delete failed: ' + err.message, 'error'); }
};
...
window.toggleVideoPromoActive = async (id, active) => {
  try {
    ...
    toast(active ? 'Video promo activated.' : 'Deactivated.', 'info');
  } catch(err) { toast('Update failed: ' + err.message, 'error'); }
};
...
// ============================================================
//  OFFERS — Full section (separate nav)
// ============================================================

function listenOffers() {
  if (STATE.unsubOffers) STATE.unsubOffers();
  STATE.unsubOffers = onSnapshot(
...
function renderOffersHistory(list) {
  const el = $('offersHistory');
  if (!el) return;
  if (!list.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-state-icon">🎁</div><div class="empty-state-text">No offers sent yet</div></div>`;
    return;
  }
  el.innerHTML = list.map(o => `
    <div class="announce-item" style="border-left:3px solid rgba(255,184,48,0.5);">
      ...
      <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end;">
        <button class="announce-delete" onclick="deleteOffer('${o.id}')" title="Delete"><i class="fa-solid fa-xmark"></i></button>
        <button class="btn btn-outline" style="padding:4px 8px;font-size:10px;" onclick="toggleOfferActive('${o.id}',${!o.active})">
...
window.sendOffer = async () => {
  ...
  if (!heading) { toast('Enter an offer heading.', 'warning'); return; }
  if (targetType === 'user' && !selectedUser) { toast('Enter a target username.', 'warning'); return; }

  btn.disabled = true; btn.innerHTML = '<span class="spinner"></span> Sending…';
  const finalTarget = targetType === 'user' ? selectedUser : 'all';

  try {
    await addDoc(collection(db, COLL.OFFERS), {
      heading, message: message || '',
      imageUrl: imageUrl || null, ctaUrl: ctaUrl || null,
...
window.deleteOffer = async id => {
  ...
  if (!yes) return;
  try { await deleteDoc(doc(db, COLL.OFFERS, id)); toast('Offer deleted.', 'info'); }
  catch(err) { toast('Delete failed: ' + err.message, 'error'); }
};
...
function addQuestionBlockFromData(data) {
  ...
  if (typeEl) {
    typeEl.value = type;
    // Trigger type change — shows/hides MCQ vs Match vs Table sections
    window.handleQTypeChange(typeEl, idx);
  }

  // Normalise options — support both options[] array and optionA-D shape
  ...
  // integer: options not applicable, already hidden by handleQTypeChange

  // Persist to Firestore draft now that fields are fully populated
  // (addQuestionBlock() already auto-saved an empty draft; refresh it with real data)
  if (STATE.activeTest) {
    _saveDraftToFirestore(idx);
  }
}
...
window.showSection = function(id) {
  ...
  if (id === 'sectionAICreate') {
    // Reset AI parse state when navigating to section
    resetAIParseState();
  }
};

// ============================================================
//  AI TEST CREATE — Groq-powered PDF/JSON extraction
// ============================================================

const GROQ_WORKER_URL = 'https://ai-analysis.untitledworld9.workers.dev';

// AI State
const AI_STATE = {
  file:      null,
  fileType:  null,  // 'pdf' | 'json'
  parsedQs:  [],    // extracted questions array
  testMeta:  null   // { exam, year, date, shift, duration, marking, testName, subjects }
};

window.selectAIExam = (exam) => {
  $('aiCuetCard').className = 'exam-selector-card' + (exam === 'cuet' ? ' selected-cuet' : '');
  $('aiJeeCard').className  = 'exam-selector-card' + (exam === 'jee'  ? ' selected-jee'  : '');
  $('aiExamCuet').checked = exam === 'cuet';
  $('aiExamJee').checked  = exam === 'jee';
  handleAIExamChange();
};

window.handleAIExamChange = () => {
  const exam = $('aiExamCuet').checked ? 'cuet' : ($('aiExamJee').checked ? 'jee' : '');
  $('aiCuetSubjectGroup').style.display = exam === 'cuet' ? 'block' : 'none';
  $('aiJeeSubjectGroup').style.display  = exam === 'jee'  ? 'block' : 'none';
  if (exam === 'cuet') { $('aiDuration').value = '60';  if ($('aiMarking')) $('aiMarking').value = '+5/-1'; }
  if (exam === 'jee')  { $('aiDuration').value = '180'; if ($('aiMarking')) $('aiMarking').value = '+4/-1'; }
};

window.handleAIFileDrop = (e) => {
  e.preventDefault();
  const f = e.dataTransfer.files?.[0];
  if (f) setAIFile(f);
  $('aiDropZone').style.borderColor = 'rgba(124,92,252,.35)';
};

window.handleAIFileSelect = (input) => {
...
function setAIFile(f) {
  ...
  if (!['pdf','json'].includes(ext)) {
    toast('Only PDF and JSON files are supported.', 'warning'); return;
  }
  if (f.size > 12 * 1024 * 1024) {
    toast('File too large. Max 10MB.', 'warning'); return;
  }
  AI_STATE.file     = f;
  AI_STATE.fileType = ext;

  $('aiFileName').textContent = f.name;
  $('aiFileSize').textContent = (f.size / 1024).toFixed(1) + ' KB';
  $('aiFileInfo').style.display = 'flex';
  $('aiDropZone').style.borderColor = 'rgba(0,229,160,.4)';
  $('aiDropZone').style.background  = 'rgba(0,229,160,.04)';
...
window.clearAIFile = () => {
  AI_STATE.file     = null;
  AI_STATE.fileType = null;
  $('aiFileInput').value = '';
  $('aiFileInfo').style.display  = 'none';
  $('aiDropZone').style.borderColor = 'rgba(124,92,252,.35)';
  $('aiDropZone').style.background  = 'rgba(124,92,252,.04)';
};

function resetAIParseState() {
  clearAIFile();
  AI_STATE.parsedQs = [];
  if ($('aiPreviewCard')) $('aiPreviewCard').style.display = 'none';
  if ($('aiParseProgress')) $('aiParseProgress').style.display = 'none';
  if ($('aiParseBar')) $('aiParseBar').style.width = '0%';
}

function setParseProgress(pct, msg) {
  $('aiParseProgress').style.display = 'block';
  $('aiParseBar').style.width = pct + '%';
  $('aiParseStatus').textContent = msg;
}

window.startAIParse = async () => {
  // Validate config
  const exam = $('aiExamCuet').checked ? 'cuet' : ($('aiExamJee').checked ? 'jee' : '');
  if (!exam) { toast('Please select an exam type first.', 'warning'); return; }

  const year    = parseInt($('aiYear').value) || new Date().getFullYear();
  const date    = $('aiDate').value;
  const shift   = $('aiShift').value;
  const marking = $('aiMarking').value;

  if (!shift) { toast('Please select a shift.', 'warning'); return; }

  // Subjects
  const subjCheckboxes = document.querySelectorAll(
    exam === 'cuet' ? 'input[name="aiCuetSubj"]:checked' : 'input[name="aiJeeSubj"]:checked'
  );
  const selectedSubjects = [...subjCheckboxes].map(cb => cb.value);
  if (!selectedSubjects.length) { toast('Select at least one subject.', 'warning'); return; }

  if (!AI_STATE.file) { toast('Please upload a PDF or JSON file.', 'warning'); return; }

  AI_STATE.testMeta = {
    exam, year, date, shift, marking,
    duration: parseInt($('aiDuration').value) || 180,
    testName: $('aiTestName').value.trim() || null,
    isFree: $('aiIsFree')?.checked || false,
    subjects: selectedSubjects
  };
...
async function parseJSONFile() {
  setParseProgress(20, 'Reading JSON file...');
  const text = await AI_STATE.file.text();
  setParseProgress(60, 'Parsing JSON structure...');
  let data;
  ...
  else throw new Error('JSON must be an array or have a "questions" array.');

  setParseProgress(90, 'Normalising questions...');
  AI_STATE.parsedQs = questions.map((q, i) => ({
  ...
  setParseProgress(100, `Done! ${AI_STATE.parsedQs.length} questions loaded.`);
}

function selectedFirstSubject() {
  const exam = $('aiExamCuet').checked ? 'cuet' : 'jee';
  const checked = document.querySelector(
    exam === 'cuet' ? 'input[name="aiCuetSubj"]:checked' : 'input[name="aiJeeSubj"]:checked'
  );
  return checked?.value || 'General';
}

async function parsePDFWithAI() {
  setParseProgress(10, 'Reading PDF file...');

  // Convert PDF to base64
  const base64 = await fileToBase64(AI_STATE.file);
  setParseProgress(25, 'Sending to Groq AI for extraction...');

  const subjects = AI_STATE.testMeta.subjects.join(', ');
  const prompt = `You are an expert at extracting Indian competitive exam questions (JEE/CUET) from PDFs. Extract ALL questions from the provided exam paper.

Return ONLY a valid JSON array (no markdown, no explanation, no code blocks) using these formats:

Normal MCQ:
{"questionNo":1,"subject":"Physics","type":"mcq","question":"Full question text","options":["Option A","Option B","Option C","Option D"],"correctAnswer":"A","section":null}

Match the Following:
{"questionNo":5,"subject":"Chemistry","type":"match","question":"Match List-I with List-II","left":["Item A","Item B","Item C","Item D"],"right":["Item I","Item II","Item III","Item IV"],"options":["(A)-(I), (B)-(II), (C)-(III), (D)-(IV)","(A)-(II), (B)-(III), (C)-(I), (D)-(IV)","(A)-(III), (B)-(IV), (C)-(II), (D)-(I)","(A)-(IV), (B)-(I), (C)-(III), (D)-(II)"],"correctAnswer":"B","section":null}

Table Based:
{"questionNo":8,"subject":"Chemistry","type":"table","question":"Based on data, identify correct option:","table":[{"header":["Column1","Column2","Column3"]},{"row":["val1","val2","val3"]},{"row":["val1","val2","val3"]}],"options":["Option A","Option B","Option C","Option D"],"correctAnswer":"C","section":null}

Integer/Numerical:
{"questionNo":12,"subject":"Mathematics","type":"integer","question":"Find the value of...","options":[],"correctAnswer":"42","section":null}

Rules:
- subject must be one of: ${subjects}
- correctAnswer: exactly "A"/"B"/"C"/"D" for MCQ/match/table; numeric string for integer
- For match questions, ALWAYS extract left[] and right[] arrays separately with individual items
- Extract ALL questions, preserve original question numbers
- Do not merge or skip any questions
- Return raw JSON array only`;

  try {
    // Use the Groq worker (Gemini-compatible format)
    ...
    setParseProgress(70, 'Processing AI response...');
    ...
    if (arrStart === -1 || arrEnd === -1) throw new Error('AI did not return a valid JSON array');
    text = text.slice(arrStart, arrEnd + 1);

    setParseProgress(85, 'Parsing extracted questions...');
    let questions;
    try { questions = JSON.parse(text); } catch(e) { throw new Error('AI returned invalid JSON: ' + e.message); }

    AI_STATE.parsedQs = questions.filter(q => q.question).map((q, i) => ({
    ...
    setParseProgress(100, `Done! ${AI_STATE.parsedQs.length} questions extracted.`);
  } catch(err) {
    // If PDF parsing fails (worker may not support it), try text extraction fallback
    setParseProgress(50, 'PDF vision failed, trying text extraction...');
    await parsePDFAsText(base64);
  }
}

async function parsePDFAsText(base64) {
  // Decode base64 to extract raw text for simple PDFs
  const prompt = `Extract ALL exam questions from this content.
Return ONLY a valid JSON array format — no markdown, no explanation:
[
  {"questionNo":1,"subject":"Physics","type":"mcq","question":"...","options":["A text","B text","C text","D text"],"correctAnswer":"A"},
  {"questionNo":5,"subject":"Chemistry","type":"match","question":"Match List-I with List-II","left":["Item A","Item B","Item C","Item D"],"right":["Item I","Item II","Item III","Item IV"],"options":["(A)-(I),(B)-(II),(C)-(III),(D)-(IV)","...","...","..."],"correctAnswer":"B"},
  {"questionNo":8,"subject":"Chemistry","type":"table","question":"Based on data...","table":[{"header":["Col1","Col2"]},{"row":["v1","v2"]},{"row":["v1","v2"]}],"options":["Option A","Option B","Option C","Option D"],"correctAnswer":"C"},
  {"questionNo":12,"subject":"Mathematics","type":"integer","question":"Find the value...","options":[],"correctAnswer":"42"}
]
Subject options: ${AI_STATE.testMeta.subjects.join(', ')}
Rules: For match questions always extract left[] and right[] arrays. Return raw JSON array only.`;

  const response = await fetch(GROQ_WORKER_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      contents: [{ parts: [{ text: `PDF base64 content: ${base64.substring(0,500)}...\n\n${prompt}` }] }],
      generationConfig: { temperature: 0.1, maxOutputTokens: 4096 }
    })
  });

  if (!response.ok) throw new Error(`AI fallback failed: HTTP ${response.status}`);
  const data = await response.json();
  let text = data?.candidates?.[0]?.content?.parts?.[0]?.text || '';
  text = text.replace(/```json\s*/gi,'').replace(/```\s*/g,'').trim();
  const arrStart = text.indexOf('[');
  const arrEnd   = text.lastIndexOf(']');
  if (arrStart === -1) throw new Error('AI could not parse PDF content. Try uploading as JSON.');
  text = text.slice(arrStart, arrEnd + 1);
...
function renderAIPreview() {
  const qs = AI_STATE.parsedQs;
  if (!qs.length) { toast('No questions found in the file.', 'warning'); return; }

  const card    = $('aiPreviewCard');
  const preview = $('aiQuestionPreview');
  const qCount  = $('aiQCount');
  const summary = $('aiParseSummary');
  if (!card || !preview) return;

  card.style.display = 'block';
  card.scrollIntoView({ behavior: 'smooth', block: 'start' });
  if (qCount) qCount.textContent = `${qs.length} Questions`;

  // Subject breakdown
  const subMap = {};
  qs.forEach(q => { subMap[q.subject] = (subMap[q.subject] || 0) + 1; });
  const subjectHTML = Object.entries(subMap).map(([sub, cnt]) =>
    `<span class="badge badge-cyan" style="font-size:11px;">${escHtml(sub)}: ${cnt}Q</span>`
  ).join(' ');

  const meta = AI_STATE.testMeta;
  if (summary) summary.innerHTML = `
    <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-bottom:12px;">
      <span class="badge ${meta.exam === 'cuet' ? 'badge-cyan' : 'badge-amber'}" style="font-size:12px;">${meta.exam.toUpperCase()}</span>
      <span class="badge badge-violet" style="font-size:11px;">${meta.year} · ${meta.shift}</span>
      <span class="badge badge-green" style="font-size:11px;">${meta.marking}</span>
      <span class="badge badge-muted" style="font-size:11px;">${meta.duration} min</span>
      ${subjectHTML}
    </div>
    <div style="font-size:12px;color:var(--text-muted);">
      <i class="fa-solid fa-circle-info" style="color:var(--accent-cyan);margin-right:4px;"></i>
      Review questions below. You can edit and re-publish after creation too.
    </div>`;

  // Render first 30 questions (performance)
  const previewQs = qs.slice(0, 30);
  preview.innerHTML = previewQs.map((q, i) => {
    const letters = ['A','B','C','D'];
    const roman   = ['I','II','III','IV','V','VI','VII','VIII'];

    // Build content block based on question type
    let contentHtml = '';

    // Match the Following — show left/right columns
    if (q.type === 'match' && q.left && q.left.length) {
      const rows = Math.max(q.left.length, (q.right || []).length);
      const rowsHtml = Array.from({length: rows}, (_, ri) => `
        <tr>
          <td style="padding:5px 10px;border:1px solid rgba(255,255,255,.06);font-size:12px;color:var(--accent-cyan);">(${letters[ri] || String.fromCharCode(65+ri)}) ${escHtml(q.left[ri] || '')}</td>
          <td style="padding:5px 10px;border:1px solid rgba(255,255,255,.06);font-size:12px;color:var(--text-secondary);">(${roman[ri] || String(ri+1)}) ${escHtml((q.right||[])[ri] || '')}</td>
        </tr>`).join('');
      contentHtml += `<div style="margin-bottom:8px;overflow-x:auto;">
        <table style="width:100%;border-collapse:collapse;">
          <thead><tr>
            <th style="padding:5px 10px;border:1px solid rgba(255,255,255,.06);font-size:10px;font-weight:700;color:var(--text-muted);text-align:left;background:rgba(0,224,255,.04);text-transform:uppercase;letter-spacing:.05em;">Column I</th>
            <th style="padding:5px 10px;border:1px solid rgba(255,255,255,.06);font-size:10px;font-weight:700;color:var(--text-muted);text-align:left;background:rgba(0,224,255,.04);text-transform:uppercase;letter-spacing:.05em;">Column II</th>
          </tr></thead>
          <tbody>${rowsHtml}</tbody>
        </table></div>`;
    }

    // Table Based — show the data table
    if (q.type === 'table' && q.table && q.table.length) {
      const headerItem = q.table.find(r => r.header);
      const dataRows   = q.table.filter(r => r.row);
      const headerHtml = headerItem
        ? `<thead><tr>${headerItem.header.map(h =>
            `<th style="padding:5px 8px;border:1px solid rgba(255,255,255,.06);font-size:10px;font-weight:700;color:var(--text-muted);background:rgba(255,184,48,.04);">${escHtml(h)}</th>`
          ).join('')}</tr></thead>` : '';
      const bodyHtml = dataRows.map(r =>
        `<tr>${r.row.map(c =>
          `<td style="padding:5px 8px;border:1px solid rgba(255,255,255,.06);font-size:11px;color:var(--text-secondary);">${escHtml(c)}</td>`
        ).join('')}</tr>`
      ).join('');
      contentHtml += `<div style="margin-bottom:8px;overflow-x:auto;">
        <table style="width:100%;border-collapse:collapse;">${headerHtml}<tbody>${bodyHtml}</tbody></table></div>`;
    }

    // Options (for all types including match/table combo strings)
    const opts = (q.options || []).map((o, oi) =>
      `<div style="font-size:12px;padding:3px 0;${letters[oi] === q.correctAnswer ? 'color:var(--accent-green);font-weight:600;' : 'color:var(--text-muted);'}">
        <span style="font-weight:700;">${letters[oi]}.</span> ${escHtml(o)}
        ${letters[oi] === q.correctAnswer ? ' <i class="fa-solid fa-check" style="font-size:10px;"></i>' : ''}
      </div>`
    ).join('');
    if (opts) contentHtml += opts;

    return `
    <div style="background:var(--bg-card-hover);border:1px solid var(--border);border-radius:10px;padding:14px;">
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;flex-wrap:wrap;">
        <span style="font-family:var(--font-mono);font-size:11px;font-weight:700;color:var(--accent-cyan);background:rgba(0,224,255,.08);padding:2px 8px;border-radius:99px;">Q.${q.questionNo}</span>
        <span class="badge badge-violet" style="font-size:10px;">${escHtml(q.subject)}</span>
        <span class="badge badge-muted" style="font-size:10px;">${q.type || 'mcq'}</span>
        ${q.correctAnswer ? `<span class="badge badge-green" style="font-size:10px;margin-left:auto;">Ans: ${q.correctAnswer}</span>` : '<span class="badge badge-amber" style="font-size:10px;margin-left:auto;">No Answer</span>'}
      </div>
      <div style="font-size:13px;color:var(--text-primary);margin-bottom:8px;line-height:1.5;">${escHtml(q.question)}</div>
      ${contentHtml}
    </div>`;
  }).join('');

  if (qs.length > 30) {
    preview.innerHTML += `<div style="text-align:center;padding:12px;font-size:12px;color:var(--text-muted);background:var(--bg-card);border-radius:8px;">
      ... and ${qs.length - 30} more questions (all will be published)
    </div>`;
  }
}

window.publishAITest = async () => {
  const qs   = AI_STATE.parsedQs;
  const meta = AI_STATE.testMeta;
  if (!qs.length || !meta) { toast('No questions to publish.', 'warning'); return; }

  const yes = await confirmModal(
    'Publish AI Test',
    `Create & publish ${meta.exam.toUpperCase()} ${meta.year} · ${meta.shift} with ${qs.length} questions? This makes the test live immediately.`
  );
  if (!yes) return;
  ...
  try {
    ...
    const testRef = await addDoc(collection(db, COLL.TESTS), testData);
    const testId  = testRef.id;

    // Step 2: Upload questions in batches
    const CHUNK = 490;
    for (let c = 0; c < Math.ceil(qs.length / CHUNK); c++) {
      const batch = writeBatch(db);
      qs.slice(c * CHUNK, (c + 1) * CHUNK).forEach((q, i) => {
        const qRef = doc(collection(db, COLL.QUESTIONS));
        batch.set(qRef, {
          testId,
          exam:          meta.exam,
      ...
      await batch.commit();
      btn.innerHTML = `<span class="spinner"></span> Uploading... ${Math.min((c+1)*CHUNK, qs.length)}/${qs.length}`;
    }

    // Clear AI state
    resetAIParseState();
    AI_STATE.parsedQs = [];
    AI_STATE.testMeta = null;
