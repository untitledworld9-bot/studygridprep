...
window.editPublishedTest = async (id) => {
  ...
  const yes = await confirmModal(
    'Edit Published Test',
    `Edit "${t.exam?.toUpperCase()} ${t.year} · ${t.shift}"? You can add/remove questions. Test will stay published.`
  );
  if (!yes) return;

  // Set as active test — enables adding questions
  STATE.activeTest     = { ...t };
  STATE.draftQuestions = [];
  STATE.qBlockCount    = 0;
  if ($('qBuilderWrap')) $('qBuilderWrap').innerHTML = '';
  ...
  refreshActiveTestBanner();
  renderSubjectNav();
  toast(`Editing: ${t.exam?.toUpperCase()} ${t.year} · ${t.shift}. Add questions and re-publish.`, 'info', 5000);
};
  ...
  el.innerHTML = list.map(h => {
    ...
    <div class="attempt-row">
      ...
      <div class="attempt-pills">
        ...
        <span class="attempt-pill pill-score">${h.score ?? '—'}/${h.total ?? '—'}</span>
      </div>
    </div>`;
  }).join('');
}
  ...
  el.innerHTML = recent.map(h => {
    ...
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
      ...
      <div style="text-align:right;flex-shrink:0;">
        <div style="font-family:var(--font-mono);font-size:12px;color:var(--accent-green);font-weight:700;">${h.score??'—'}/${h.total??'—'}</div>
        <div style="font-size:10px;color:var(--text-muted);">${formatTimestamp(h.submittedAt||h.date)}</div>
      </div>
    </div>`;
  }).join('');
...
window.loadTopScorerTests = function() {
  ...
  testSel.innerHTML = `<option value="">— Select Test —</option>` +
    tests.map(t =>
      `<option value="${escHtml(t.id)}">${escHtml(t.exam.toUpperCase())} ${t.year || ''} · ${escHtml(t.shift || '')}</option>`
    ).join('');
  ...
  const rowsHTML = list.map((h, i) => {
    ...
    <div style="display:flex;align-items:center;gap:12px;border-bottom:1px solid var(--border);${rowBg}">
      ...
      <div style="text-align:right;flex-shrink:0;min-width:92px;">
        ...
        <div style="font-size:10px;color:var(--text-muted);">/ ${h.total ?? '—'}</div>
