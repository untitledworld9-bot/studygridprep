...
function listenUsers() {
  ...
  const unsub = onSnapshot(
    ...
    snap => {
      // Deduplicate by doc ID to prevent phantom double-entries
      const seen = new Map();
      snap.docs.forEach(d => { seen.set(d.id, { id: d.id, ...d.data() }); });
      STATE.allUsers = [...seen.values()];
      updateUserStats();
      renderUserTable();
      renderDashRecent();
...
window.deleteUser = async (uid, name) => {
  const yes = await confirmModal(
    "Delete User",
    `Permanently delete "${name}" from Firestore? This removes their data from the users collection. Firebase Auth account is not affected.`
  );
  if (!yes) return;
  try {
    ...
    <table style="width:100%;border-collapse:collapse;">
      <thead>
        <tr style="border-bottom:1px solid var(--border);">
          ...
          <th style="padding:10px 12px;text-align:left;font-size:12px;color:var(--text-muted);font-weight:600;">Timer XP</th>
          <th style="padding:10px 12px;text-align:left;font-size:12px;color:var(--text-muted);font-weight:600;">Level</th>
          <th style="padding:10px 12px;text-align:left;font-size:12px;color:var(--text-muted);font-weight:600;">Status</th>
        </tr>
      </thead>
      <tbody>
        ${users.map((u, i) => {
          ...
          // Show ONLY timer XP (not combined playlist/todo XP)
