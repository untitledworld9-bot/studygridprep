...
function getCurrentPageFull() {
  ...
  return "index.html"; // default = home
}

async function initPresence() {
  // Resolve UID — wait for firebase auth via UW core, or fallback to localStorage
  let uid = null;
  ...
  // Fallback chain:
  // uwUid  = saved by script.js (user.uid) on timer.html
  // userId = some pages save this
  // userName = displayName/email — the doc key script.js uses — CRITICAL fallback
  ...
  const isTimerPage = page === "timer.html";

  // On timer.html, script.js owns status ("Focusing 👋" / "Online")
  // DO NOT override it — just update page + lastActive
  const presenceData = {
    currentPage: page,
    lastActive:  Date.now(),
    ...(!isTimerPage && { status: "online" })
  };

  try {
    await setDoc(doc(db, "users", uid), presenceData, { merge: true });
  } catch(e) { console.warn("[Presence] init failed:", e); }

  // Heartbeat every 60s
  _presenceHB = setInterval(async () => {
    try {
      const curPage   = getCurrentPageFull();
      const isTimer   = curPage === "timer.html";
      const hbData    = {
        lastActive:  Date.now(),
        currentPage: curPage,
      };
      // Only update status on non-timer pages
      if (!isTimer) hbData.status = "online";
      await updateDoc(doc(db, "users", uid), hbData);
    } catch {}
  }, 60_000);
  ...
  window._uwPresence = {
    ...
    // Call setFocusing(true) when timer starts, setFocusing(false) when stopped
