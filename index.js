...
function markSeen(type, id) {
  ...
  const entry = { name: userName || userEmail, email: userEmail, at: Date.now() };
...
function renderPromotionBanner(data) {
  ...
  // Full image — takes all available height
  if (data.imageUrl) {
    const img = document.createElement("img");
    img.src = data.imageUrl;
    img.alt = data.title || "";
  ...
  // Content block — only shown if there is title/body/cta
  // message is optional per your instructions
  const hasContent = data.title || data.body || data.cta;
  if (hasContent) {
    const content = document.createElement("div");
    content.style.cssText = "padding:18px 20px 22px;text-align:center;background:" + (data.bgColor || "#0d1117") + ";flex-shrink:0;";

    if (data.title) {
      const t = document.createElement("div");
      t.style.cssText = `font-size:18px;font-weight:800;color:#fff;line-height:1.3;margin-bottom:6px;`;
    ...
    if (data.body) {
      ...
      b.style.cssText = `font-size:13px;color:rgba(255,255,255,0.65);line-height:1.6;margin-bottom:14px;`;
...
function showOfferOverlay(data) {
  ...
  if (data.message) {
    ...
    m.style.cssText = `font-size:13px;color:rgba(255,255,255,0.7);line-height:1.6;margin-bottom:14px;`;
    m.textContent = data.message;
    content.appendChild(m);
  }

  // Countdown timer (if durationMinutes set)
  if (data.durationMinutes && data.durationMinutes > 0) {
