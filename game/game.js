...
(async()=>{try{const fb=await import('../firebase.js');_db=fb.db;_auth=fb.auth;_setDoc=fb.setDoc;_doc=fb.doc;_getDoc=fb.getDoc;_serverTimestamp=fb.serverTimestamp;fb.onAuthStateChanged(fb.auth,user=>{_firebaseUser=user;if(user)loadCloudProfile(user);});}catch(e){console.warn('[FR] offline');}})();
