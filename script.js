...
async function _syncTimerLeaderboard(focusMinsDelta, xpDelta) {
  ...
  try {
    ...
    if (focusMinsDelta > 0) {
      update.focusTime       = increment(focusMinsDelta); // cumulative total (never resets)
      update.weeklyFocusTime = increment(focusMinsDelta); // weekly (resets with weeklyTimerXP)
    }
    if (xpDelta > 0) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (confirmJoin) {
    confirmJoin.onclick = async () => {
      ...
      // Increment totalJoined in room doc
